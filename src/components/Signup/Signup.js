...
function Signup() {
  ...
  const registerUser = (formData) => {
    firebase
      .auth()
      .createUserWithEmailAndPassword(formData.userEmail, formData.userPassword)
      .then((data) => {
        data.user
          .updateProfile({
  ...
  return (
    <form className="w-1/2 p-4" onSubmit={handleSubmit(registerUser)}>
      ...
      <div className="mb-6 text-sm text-center">
        Alerady have an account, then{" "}
        <Link to="/login" className="text-indigo-500 hover:underline">
          login
        </Link>
      </div>
      {showVerifyMsg && (
        <div className="bg-white text-indigo-500 p-4 rounded-md shadow-lg flex items-center justify-around">
